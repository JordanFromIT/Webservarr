...
WebServarr ships a precompiled Tailwind CSS stylesheet at `app/static/css/tailwind.css` — there is no runtime CDN and no Node.js dependency in the container. **Downloaders never need to rebuild this file**; the committed output is used as-is.

You only need to rebuild it if you are modifying the app itself and add new Tailwind utility classes to an HTML page or a file in `app/static/js/`. The stylesheet is generated by scanning those files for class names, so a class that doesn't already appear somewhere in `app/static/**/*.html` or `app/static/js/**/*.js` won't be in the CSS until you rebuild.

Rebuild with the Tailwind standalone CLI (no Node install required — it's a single self-contained binary, pinned to a fixed version so builds are reproducible):

```bash
# Download the pinned standalone CLI (once) -- keep it outside the repo
curl -sLO https://github.com/tailwindlabs/tailwindcss/releases/download/v3.4.17/tailwindcss-linux-x64
chmod +x tailwindcss-linux-x64

# From the repo root, rebuild the stylesheet
./tailwindcss-linux-x64 -c tailwind.config.js \
  -i app/static/css/tailwind.src.css -o app/static/css/tailwind.css --minify
```

Commit the regenerated `app/static/css/tailwind.css` along with your HTML/JS changes. If a class name is built dynamically at runtime (e.g. `'prefix-' + variable`) instead of appearing as a literal string, add it to the `safelist` array in `tailwind.config.js` first, or refactor the call site to use full literal class strings — otherwise the scanner will never see it and the class will be missing from the build.
